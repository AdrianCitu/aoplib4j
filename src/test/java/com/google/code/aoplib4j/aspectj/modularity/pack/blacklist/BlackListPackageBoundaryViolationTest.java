...
import com.google.code.aoplib4j.aspectj.modularity.pack.blacklist.forbiddenpackage.ForbiddenClass;
import com.google.code.aoplib4j.aspectj.modularity.pack.whitelist.WhiteListPackageCalledClass;
import com.google.code.aoplib4j.aspectj.modularity.pack.whitelist.allowedpackage.AllowedClass;
 ...
 *
 */
public class BlackListPackageBoundaryViolationTest extends TestCase {
    ...
    /**
     * Test the calls of the {@link ForbiddenClass} to the 
     * {@link WhiteListPackageCalledClass}. 
     * The calls should raise an exception since the
     * {@link ForbiddenClass} package is not in the list of allowed packages.
     */
    @SuppressWarnings(value={"all"})
    public final void testViolatePackageBoundary() {
        try{
            forbiddenClass.voidMethod();
            fail("An IllegalAccessError should be thrown because the"
                    + forbiddenClass.getClass()
                    + " violated the package call boundary.");
        } catch (IllegalAccessError iae) {
            //nothing to do
        }
        
        try{
            ForbiddenClass.staticVoidMethod();
            fail("An IllegalAccessError should be thrown because the"
                    + forbiddenClass.getClass()
                    + " violated the package call boundary.");
        } catch (IllegalAccessError iae) {
